...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            def train_step(input_problem, input_kc, x_answer, target_id, target_kc, target_index, target_correctness):
            ...
            def validation_step(input_problem, input_kc, x_answer, target_id, target_kc, target_index, target_correctness):
            ...
            for iii in range(FLAGS.epochs):
                ...
                while(index+FLAGS.batch_size <= data_size):
                    ...
                    input_kc = np.ones((FLAGS.batch_size, max_num_steps, max_num_skills)) * 0.01
                    x_answer = np.zeros((FLAGS.batch_size, max_num_steps))
                    target_id = np.zeros((FLAGS.batch_size, max_num_steps))
                    target_kc = np.ones((FLAGS.batch_size, max_num_steps, max_num_skills)) * 0.01
                    target_correctness = []
                    target_index = []
                    ...
                    pred = train_step(input_problem, input_kc,  x_answer, target_id, target_kc, target_index, target_correctness)
                    for p in pred:
                    ...
                    while(index+FLAGS.batch_size <= data_size):
                        ...
                        for i in range(FLAGS.batch_size):
                            ...
                            for j in range(len_seq-1):
                                ...
                                actual_labels.append(int(correctness[j+1]))
                        index += FLAGS.batch_size
                        pred  = validation_step(input_problem, input_kc, x_answer, target_id, target_kc, target_index, target_correctness)
                        for p in pred:
                            pred_labels.append(p)

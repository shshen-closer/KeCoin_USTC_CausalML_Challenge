...
def test():
    ...
    test_students = np.load("data/test0.npy", allow_pickle=True)
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            pro_embedding = graph.get_operation_by_name("pro_embedding").outputs[0]
            ...
            while(index+FLAGS.batch_size <= data_size):
                ...
                construct_b = sess.run(pro_embedding, feed_dict)
                print(np.shape(pro_embedding))
                np.save('construct.npy', np.array(construct_b))
                break

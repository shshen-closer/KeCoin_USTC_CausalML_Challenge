...
def test():
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            while(index+FLAGS.batch_size <= data_size):
                ...
                input_kc_b = np.ones((FLAGS.batch_size, max_num_steps, max_num_skills)) * 0.0
                x_answer_b = np.zeros((FLAGS.batch_size, max_num_steps))
                target_id_b = np.zeros((FLAGS.batch_size, max_num_steps))
                target_kc_b = np.ones((FLAGS.batch_size, max_num_steps, max_num_skills)) * 0.0
    ...
    ooo = []
    for sss in range(0, len(jiedian), 115):
        c_i = int(sss / 115)
        score = predicts[sss:sss+115]
       # score = np.array(score)
        #score = np.greater(score,0) 
       # score = score.astype(int)
        score = list(score)
        score = score[:c_i] + [0] + score[c_i:]
        ooo.append(score)
            
    np.save('adj_matrix_o.npy', np.array(ooo))
    print(np.mean(ooo))
    logger.info("Done.")
